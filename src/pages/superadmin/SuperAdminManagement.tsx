...
export function SuperAdminManagement() {
  ...
  const [adminToDelete, setAdminToDelete] = useState<SuperAdminUser | null>(null);
  ...
  React.useEffect(() => {
    const load = async () => {
      try {
        ...
        setLoadError('Failed to load super admin users. Please try refreshing the page.');
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const isFormValid =
    adminFirstName.trim() && adminLastName.trim() && adminEmail.trim() && !emailError;
  ...
  const handleInviteAdmin = async () => {
    ...
    if (admins.some((a) => a.email.toLowerCase() === adminEmail.toLowerCase())) {
    ...
    setIsInviting(true);
    try {
      await inviteSuperAdmin(adminEmail.trim(), adminFirstName.trim(), adminLastName.trim());
      ...
      if (errorText.includes('different role') || errorText.includes('Already registered')) {
  ...
  const handleResendInvite = async (admin: SuperAdminUser) => {
    setResendingAdminId(admin.id);
    try {
      await resendSuperAdminInvite(admin.id);
  ...
  const handleUpdateAdmin = async () => {
    ...
    try {
      ...
      setAdmins(
        admins.map((a) =>
          a.id === selectedAdmin.id
            ? { ...a, first_name: adminFirstName.trim(), last_name: adminLastName.trim() }
            : a
        )
      );
      ...
      showToast('error', 'Failed to update super admin');
    }
  };

  const handleViewAdmin = (admin: SuperAdminUser) => {
    setSelectedAdmin(admin);
    setEmailError('');
    setIsViewDialogOpen(true);
  };

  const handleDeleteAdmin = (admin: SuperAdminUser) => {
  ...
  const confirmDeleteAdmin = async () => {
    ...
    try {
      ...
      showToast('error', 'Failed to delete super admin');
    }
  };
      ...
      <AdminLayout>
        <div className="flex items-center justify-center min-h-[400px] bg-white rounded-2xl border border-slate-100 shadow-xs mt-12 sm:mt-10 p-12 max-w-7xl mx-auto">
          <div className="text-center animate-pulse">
            <div className="animate-spin rounded-full border-b-2 border-[#0F2D52] mx-auto mb-3 h-8 w-8" />
            <p className="text-slate-500 text-sm font-semibold">Loading super admin users...</p>
          </div>
        </div>
      </AdminLayout>
    );
    ...
    <AdminLayout>
      <div className="container mx-auto px-2 sm:px-4 py-0 sm:pt-12 max-w-7xl space-y-6 pb-12">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between my-5 gap-4 mt-16 sm:mt-4 bg-white p-6 rounded-2xl border border-slate-100 shadow-xs">
          <div>
            <h1 className="text-xl sm:text-2xl font-extrabold text-slate-950 tracking-tight">Super Admin Management</h1>
            <p className="text-xs sm:text-sm text-slate-400 font-semibold mt-0.5">
              Manage platform-level super administrator accounts and access.
            </p>
          </div>
          <div>
            <Button
              ...
              size="sm"
              onClick={() => { setEmailError(''); setIsAddDialogOpen(true); }}
            >
              <Plus className="w-4 h-4 mr-2" /> Invite Super Admin
            </Button>
          </div>
        </div>
        ...
        {loadError && (
          <div className="rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700 flex items-center justify-between animate-fade-in">
            ...
            <Button
              ...
              variant="outline"
              className="h-8 px-3 text-xs bg-white text-red-700 border-red-200 hover:bg-red-50 rounded-lg"
            >
              Refresh Page
            </Button>
          </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden hover:shadow-md transition-all duration-300 animate-fade-in-up" style={{ animationDelay: '100ms' }}>
          <div className="px-5 py-4 border-b border-slate-50 bg-slate-50/50">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
              <div>
                <h2 className="text-sm font-bold text-slate-900">Super Administrator Directory</h2>
                <p className="text-xs text-slate-400 mt-0.5 font-semibold">
                  Showing {filteredAdmins.length} of {totalAdmins} super admin{totalAdmins === 1 ? '' : 's'}
                </p>
              </div>
              ...
              <div className="flex items-center gap-1 bg-slate-100/80 p-1 rounded-xl border border-slate-200/50 shadow-xs">
                <button
                  ...
                  className={`flex items-center gap-1 px-2.5 sm:px-3 py-2 rounded-lg text-[10px] font-bold transition-all ${viewMode === 'table'
                ...
                <button
                  ...
                  className={`flex items-center gap-1 px-2.5 sm:px-3 py-2 rounded-lg text-[10px] font-bold transition-all ${viewMode === 'card'
                      ? 'bg-white text-[#0F2D52] shadow-xs'
                      : 'text-slate-500 hover:text-slate-800 hover:bg-slate-50/50'
                    }`}
                >
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              ...
              <div className="flex items-center gap-2">
                ...
                {(searchTerm || statusFilter !== 'all') && (
                  <Button
                    variant="ghost"
                    onClick={() => { handleSearchChange(''); handleStatusFilterChange('all'); }}
          ...
          {viewMode === 'card' ? (
            <MobileCardList
              className="p-4"
              loading={isLoading}
              loadingMessage="Loading super admins..."
              emptyMessage="No super admins found. Try adjusting your search or filters."
                    ...
                    <div>
                        ...
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end" className="bg-white rounded-xl border border-slate-100 shadow-xl z-50">
                            ...
                            <DropdownMenuItem className="cursor-pointer font-medium text-xs text-slate-700" onClick={() => handleEditAdmin(admin)}>
                              <Edit className="w-4 h-4 mr-2 text-slate-400" /> Edit Super Admin
                            </DropdownMenuItem>
                            {!admin.is_verified && (
                              <DropdownMenuItem
                                className="cursor-pointer font-medium text-xs text-slate-700"
                                disabled={resendingAdminId === admin.id}
                                onClick={(e) => { e.preventDefault(); handleResendInvite(admin); }}
                              >
                            ...
                            <DropdownMenuItem className="cursor-pointer font-medium text-xs text-red-600 focus:text-red-600 focus:bg-red-50" onClick={() => handleDeleteAdmin(admin)}>
                              <Trash2 className="w-4 h-4 mr-2" /> Delete Super Admin
                            </DropdownMenuItem>
                          </DropdownMenuContent>
                        </DropdownMenu>
                      </div>
                      ...
                      <div className="flex items-center justify-between border-t border-slate-100 pt-3 mt-1">
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className="inline-flex items-center text-[10px] font-bold px-2 py-0.5 rounded-full bg-[#EFF5FB] text-[#0F2D52] border border-blue-200/50">
              ...
              rows={paginatedAdmins.map((admin) => {
                ...
                return (
                  <tr key={admin.id} className="border-b border-slate-50 hover:bg-[#F8FAFC] transition-colors duration-150">
                    ...
                    <td className="py-4 px-4">
                      <span className="inline-flex items-center text-xs font-semibold px-2.5 py-0.5 rounded-full bg-[#EFF5FB] text-[#0F2D52] border border-blue-200/50">
                        {admin.role.charAt(0).toUpperCase() + admin.role.slice(1)}
                      </span>
                    </td>
                    <td className="py-4 px-4">
        ...
        {/* ── Invite Dialog ──────────────────────────────────────────────── */}
        <Dialog open={isAddDialogOpen} onOpenChange={(open) => { if (!open) { setIsAddDialogOpen(false); resetForm(); } }}>
          <DialogContent className="w-[95vw] max-w-md max-h-[90vh] overflow-y-auto no-scrollbar rounded-2xl shadow-lg border border-slate-100 bg-white p-6">
            <DialogHeader className="mb-4">
              <DialogTitle className="text-lg font-bold text-slate-900">Invite New Super Admin</DialogTitle>
              <p className="text-xs text-slate-500 font-medium">Send an email invitation to register a new platform-level super administrator.</p>
            </DialogHeader>
            <div className="space-y-4 py-2">
              ...
              <div>
                ...
                <Input
                  ...
                  className={`w-full h-10 rounded-xl border text-sm focus:outline-none focus:ring-2 transition-all bg-white ${emailError
            ...
            <DialogFooter className="mt-6 flex flex-col sm:flex-row gap-2 sm:gap-3">
              <Button
                ...
                className="w-full sm:w-auto h-10 text-sm rounded-xl border border-slate-200 bg-white text-slate-700 hover:bg-slate-50 transition-all font-medium"
              >
                Cancel
              </Button>
              <Button
                ...
                className="w-full sm:w-auto h-10 text-sm rounded-xl bg-[#0F2D52] hover:bg-[#1c477c] text-white transition-all font-semibold disabled:opacity-50 flex items-center justify-center gap-1.5"
              >
        ...
        {/* ── Edit Dialog ────────────────────────────────────────────────── */}
        <Dialog open={isEditDialogOpen} onOpenChange={(open) => { if (!open) { setIsEditDialogOpen(false); setSelectedAdmin(null); resetForm(); } }}>
          <DialogContent className="w-[95vw] max-w-md max-h-[90vh] overflow-y-auto no-scrollbar rounded-2xl shadow-lg border border-slate-100 bg-white p-6">
            <DialogHeader className="mb-4">
              <div className="w-10 h-10 rounded-xl bg-[#EFF5FB] text-[#0F2D52] flex items-center justify-center mb-3">
                <Edit className="w-5 h-5" />
              </div>
              <DialogTitle className="text-lg font-bold text-slate-900">Edit Super Admin Details</DialogTitle>
              <p className="text-xs text-slate-500 font-medium">Update the name or contact information for this super administrator.</p>
            </DialogHeader>
            <div className="space-y-4 py-2">
              <div>
                <label className="block text-xs font-bold uppercase tracking-wider text-slate-400 mb-1.5">First Name</label>
            ...
            <DialogFooter className="mt-6 flex flex-col sm:flex-row gap-2 sm:gap-3">
              ...
              <Button
                ...
                className="w-full sm:w-auto h-10 text-sm rounded-xl bg-[#0F2D52] hover:bg-[#1c477c] text-white transition-all font-semibold"
              >
                Update Super Admin
              </Button>
            </DialogFooter>
        ...
        {/* ── View Details Dialog ────────────────────────────────────────── */}
        <Dialog open={isViewDialogOpen} onOpenChange={(open) => { if (!open) { setIsViewDialogOpen(false); setSelectedAdmin(null); } }}>
          <DialogContent className="w-[95vw] max-w-md max-h-[90vh] overflow-y-auto no-scrollbar rounded-2xl shadow-lg border border-slate-100 bg-white p-6">
            <DialogHeader className="mb-4">
              <div className="w-10 h-10 rounded-xl bg-[#EFF5FB] text-[#0F2D52] flex items-center justify-center mb-3">
                <Eye className="w-5 h-5" />
              </div>
              <DialogTitle className="text-lg font-bold text-slate-900">Super Admin Details</DialogTitle>
              <p className="text-xs text-slate-500 font-medium">Comprehensive information about this platform administrator.</p>
            </DialogHeader>
            {selectedAdmin && (
              <div className="space-y-5 py-2">
                <div className="flex items-center space-x-4 p-3 bg-slate-50/50 border border-slate-100 rounded-xl">
                  <AvatarInitials
                    initials={`${selectedAdmin.first_name?.[0] || ''}${selectedAdmin.last_name?.[0] || ''}`.toUpperCase()}
                    size="lg"
                  />
                  <div className="min-w-0">
                ...
                <div className="grid grid-cols-2 gap-4 border-t border-slate-50 pt-4">
                  <div>
                    ...
                    <Badge variant={selectedAdmin.is_verified ? 'success' : 'secondary'} className="text-[10px] font-bold rounded-full px-2 py-0.5">
                      {selectedAdmin.is_verified ? 'Active' : 'Pending'}
                    </Badge>
                  </div>
                  <div>
        ...
        <Dialog open={isDeleteDialogOpen} onOpenChange={(open) => { if (!open) { setIsDeleteDialogOpen(false); setAdminToDelete(null); } }}>
          <DialogContent className="w-[95vw] max-w-md max-h-[90vh] overflow-y-auto no-scrollbar rounded-2xl shadow-lg border border-slate-100 bg-white p-6">
            <DialogHeader className="mb-4">
              <div className="w-10 h-10 rounded-xl bg-red-50 text-red-600 flex items-center justify-center mb-3">
                <Trash2 className="w-5 h-5" />
              </div>
              <DialogTitle className="text-lg font-bold text-slate-900">Delete Super Administrator</DialogTitle>
              <p className="text-xs text-slate-500 font-medium">Removing this super admin will revoke their platform access immediately.</p>
            </DialogHeader>
            <div className="py-2">
            ...
            <DialogFooter className="mt-6 flex flex-col sm:flex-row gap-2 sm:gap-3">
              ...
              <Button
                ...
                className="w-full sm:w-auto h-10 text-sm rounded-xl bg-red-600 hover:bg-red-700 text-white font-semibold transition-all"
              >
                Delete Super Admin
              </Button>
            </DialogFooter>
