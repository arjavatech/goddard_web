...
function SnapThumb({ url, label }: { url?: string; label: string }) { if (!url) return <span className="text-slate-400 text-xs">—</span>; return <img src={url} alt={label} className="h-10 w-10 rounded-lg object-cover cursor-pointer border border-slate-200 hover:opacity-80 transition-opacity" onClick={() => window.open(url, '_blank')} />; }
