...
export function MyDocuments({ audience }: { audience: Audience }) {
  ...
  const upload = async (file?: File) => {
    if (!file || !uploading || isUploading) return;
  ...
  const content = <div className={`mx-auto w-full max-w-6xl space-y-6 mt-16 transition-opacity duration-200 ${isUploading ? 'pointer-events-none opacity-60' : ''}`}>
    <section className="rounded-2xl border border-slate-100 bg-white p-6 shadow-xs">
      <div className="flex flex-wrap items-start justify-between gap-4">
        ...
        <Button variant="outline" onClick={() => void load()} disabled={loading || isUploading} className="rounded-xl text-xs font-bold disabled:opacity-50">
        ...
        {isParent && selectedChildId && 
          <p className="-mt-2 text-sm text-slate-500">
            Showing document requirements for <span className="font-semibold text-slate-700">{selectedChildId}</span>.</p>}<section className="grid grid-cols-2 gap-4 sm:grid-cols-4"><Summary label="Upload required" value={summary.required} /><Summary label="Submitted" value={summary.submitted} /><Summary label="Approved" value={summary.approved} /><Summary label="Re-upload needed" value={summary.rejected} /></section>{loading ? <div className="py-16 text-center text-sm text-slate-500">Loading documents…</div> : !visibleItems.length ? <div className="rounded-2xl border border-slate-100 bg-white py-16 text-center text-sm text-slate-500">No document requests are currently assigned.</div> : <section className="grid auto-rows-fr items-stretch gap-5 sm:grid-cols-2">{visibleItems.map(item => <DocumentCard key={item.id} item={item} isUploading={isUploading} onView={() => void openDetails(item)} onUpload={() => startUpload(item)} />)}</section>}<input ref={input} type="file" accept="application/pdf,image/jpeg,image/png" className="hidden" onChange={event => { void upload(event.target.files?.[0]); event.currentTarget.value = ''; }} />{uploadError && <div className="fixed top-14 left-1/2 z-50 -translate-x-1/2 rounded-xl bg-rose-600 px-4 py-3 text-sm font-medium text-white shadow-lg"><XCircle className="mr-2 inline h-4 w-4" />{uploadError}</div>}{uploadSuccess && <div className="fixed top-24 left-1/2 z-50 -translate-x-1/2 rounded-xl bg-emerald-600 px-4 py-3 text-sm font-medium text-white shadow-lg flex items-center"><CheckCircle2 className="mr-2 h-4 w-4" />{uploadSuccess}</div>}{isUploading && <div className="fixed top-1/2 left-1/2 z-50 -translate-x-1/2 -translate-y-1/2 rounded-xl bg-slate-800 px-6 py-4 text-sm font-medium text-white shadow-2xl flex items-center"><RefreshCw className="mr-3 h-5 w-5 animate-spin" />Uploading...</div>}<DocumentDetails item={selected} history={history} onClose={() => setSelected(null)} onUpload={() => { if (selected) { setSelected(null); startUpload(selected); } }} /></div>;
...
function DocumentCard({ item, onView, onUpload, isUploading }: { item: DocumentAssignment; onView: () => void; onUpload: () => void; isUploading?: boolean }) { const status = assignmentStatus(item); const action = status === 'rejected' ? 'Re-upload document' : status === 'submitted' ? 'Replace document' : 'Upload document'; return <Card className={`h-full rounded-2xl bg-white shadow-xs ${statusBorder[status]}`}><CardContent className="flex h-full flex-col p-5"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><h2 className="font-bold text-slate-900">{item.document_name}</h2><p className="mt-1 text-sm text-slate-500">{item.instructions}</p></div><span className={`shrink-0 rounded-full px-2.5 py-1 text-xs font-bold ${statusStyle[status]}`}>{statusLabel[status]}</span></div><div className="mt-4 border-y border-slate-100 py-3 text-sm text-slate-600"><p>Due date: <span className="font-semibold">{date(item.due_date)}</span></p>{item.submitted_at ? <p className="mt-1 text-xs text-slate-500">Submitted {date(item.submitted_at)} · Version {item.version_count}</p> : <p className="mt-1 text-xs text-slate-400">Not submitted yet</p>}</div><StatusMessage item={item} status={status} /><div className="mt-auto flex flex-wrap gap-2 pt-5">{status !== 'approved' && <Button disabled={isUploading} onClick={onUpload} className="order-1 rounded-xl bg-gradient-to-br from-[#0F2D52] to-[#1E4B83] text-white hover:text-white [&_svg]:text-white disabled:opacity-50"><Upload className="mr-2 h-4 w-4" />{action}</Button>}{item.latest_submission_id && <Button disabled={isUploading} variant="outline" onClick={onView} className="order-2 rounded-xl disabled:opacity-50"><Eye className="mr-2 h-4 w-4" />View details</Button>}</div></CardContent></Card>; }
