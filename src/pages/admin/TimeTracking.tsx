...
import { BarChart3, CalendarDays, ChevronDown, Clock, Clock3, Download, FileText, Filter, Grid2X2, History, List, Loader2, Pencil, Plus, Search, Trash2, Users, X } from 'lucide-react';
...
export function TimeTracking() {
  ...
  const [tab, setTab] = useState<Tab>(() => {
    ...
    return (tabParam === 'today' || tabParam === 'daywise' || tabParam === 'range' || tabParam === 'pending') ? tabParam : 'today';
  });
  ...
  return (
    <AdminLayout>
      <main className="min-h-full bg-[#f6f9fd] px-4 py-6 sm:px-6 lg:px-8">
        ...
        <section className="overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-sm"><div className="border-b border-slate-100 bg-slate-50/50 px-5"><nav className="flex gap-6 overflow-x-auto">{tabs.map(({ key, label, icon: Icon }) => <button key={key} onClick={() => { setTab(key); setSearchParams({ tab: key }, { replace: true }); }} className={`flex shrink-0 items-center gap-2 border-b-2 px-1 py-4 text-sm font-bold ${tab === key ? 'border-[#1a6fc4] text-[#0F2D52]' : 'border-transparent text-slate-500 hover:text-[#0F2D52]'}`}><Icon className="h-4 w-4" />{label}</button>)}</nav></div>
        <div className="p-5">
...
function SnapThumb({ url, label }: { url?: string; label: string }) { const [open, setOpen] = useState(false); if (!url) return <span className="text-slate-400 text-xs">—</span>; return <><img src={url} alt={label} className="h-10 w-10 rounded-lg object-cover cursor-pointer border border-slate-200 hover:opacity-80 transition-opacity" onClick={() => setOpen(true)} /><Dialog open={open} onOpenChange={setOpen}><DialogContent className="sm:max-w-lg flex flex-col items-center gap-4"><DialogHeader><DialogTitle>{label}</DialogTitle></DialogHeader><img src={url} alt={label} className="max-h-[70vh] w-full object-contain rounded-xl" /><DialogFooter><Button className="bg-[#0F2D52] text-white hover:bg-[#173d69] hover:text-white" onClick={() => setOpen(false)}>Close</Button></DialogFooter></DialogContent></Dialog></>; }
...
function ReportTable({ items, onEdit, onDelete, onHistory, pending, canManage }: { items: AttendanceReport[]; onEdit: (item: AttendanceReport) => void; onDelete: (item: AttendanceReport) => void; onHistory: (item: AttendanceReport) => void; pending: boolean; canManage: boolean }) { const headers = ['Employee', 'Date', 'Check In', 'Check-In Photo', 'Check Out', 'Check-Out Photo', 'Worked', 'Last Modified By', ...(canManage ? ['Action'] : [])]; return <div className="mt-6 overflow-x-auto rounded-xl border border-slate-100"><table className="w-full min-w-[1024px] text-sm"><thead className="bg-slate-50/80"><tr>{headers.map((header, index) => <th key={header} className={`border-y border-slate-200/85 px-4 py-3.5 text-left text-xs font-bold uppercase tracking-wider text-slate-500 ${canManage && header === 'Action' ? 'text-center' : ''}`}>{header}</th>)}</tr></thead><tbody>{items.map((item, index) => <tr className="border-b border-slate-50 transition-colors hover:bg-[#F8FAFC]" style={item.last_modified_by === 'Auto Checked Out' ? { backgroundColor: 'rgba(246, 235, 97, 0.5)' } : undefined} key={`${item.emp_id}-${item.check_in_time}-${index}`}><td className="px-4 py-4"><p className="font-bold text-[#0F2D52]">{item.name || 'Employee'}</p><p className="text-xs text-slate-400">{item.email || '—'}</p></td><td className="px-4 py-4 text-slate-600">{item.date || '—'}</td><td className="px-4 py-4 text-slate-600">{displayTime(item.check_in_time)}</td><td className="px-4 py-4 flex items-center justify-center"><SnapThumb url={item.check_in_snap} label="Check-in photo" /></td><td className="px-4 py-4">{item.check_out_time ? <span className="text-slate-600">{displayTime(item.check_out_time)}</span> : <span className="rounded-full bg-amber-50 px-2.5 py-1 text-xs font-bold text-amber-700">Pending</span>}</td><td className="px-4 py-4 flex items-center justify-center"><SnapThumb url={item.check_out_snap} label="Check-out photo" /></td><td className="px-4 py-4 font-semibold text-slate-700">{item.time_worked || '—'}</td><td className="px-4 py-4 text-slate-600">{displayModifiedBy(item.last_modified_by)}</td>{canManage && <td className="px-4 py-4"><div className="flex justify-center gap-2"><Button size="sm" variant="outline" onClick={() => onEdit(item)} title={pending ? 'Complete & Edit' : 'Edit'}><Pencil className="h-3.5 w-3.5" /></Button><Button size="sm" variant="outline" onClick={() => onHistory(item)} title="History"><History className="h-3.5 w-3.5" /></Button><Button size="sm" variant="outline" className="border-red-200 text-red-700 hover:bg-red-50 hover:text-red-800" onClick={() => onDelete(item)} title="Delete"><Trash2 className="h-3.5 w-3.5" /></Button></div></td>}</tr>)}</tbody></table></div>; }
function ReportCard({ item, onEdit, onDelete, onHistory, pending, canManage }: { item: AttendanceReport; onEdit: () => void; onDelete: () => void; onHistory: () => void; pending: boolean; canManage: boolean }) { const [expanded, setExpanded] = useState(false); return <div className="rounded-2xl border border-slate-100 p-4 sm:p-5 shadow-xs transition-all hover:shadow-md bg-white overflow-hidden" style={item.last_modified_by === 'Auto Checked Out' ? { backgroundColor: 'rgba(246, 235, 97, 0.5)' } : undefined}><div className="min-w-0"><p className="font-bold text-[#0F2D52] truncate">{item.name || 'Employee'}</p><p className="mt-1 text-xs font-medium text-slate-400 truncate">{item.email || '—'}</p></div><dl className="mt-4 grid gap-2 border-t border-slate-100 pt-3 text-xs sm:text-sm text-slate-600"><div className="flex justify-between gap-2 min-w-0"><dt className="flex-shrink-0">Date</dt><dd className="text-right truncate">{item.date || '—'}</dd></div><div className="flex justify-between gap-2 min-w-0"><dt className="flex-shrink-0">Check in</dt><dd className="text-right truncate">{displayTime(item.check_in_time)}</dd></div><div className="flex justify-between gap-2 min-w-0 items-center"><dt className="flex-shrink-0">Check-in photo</dt><dd className="text-right"><SnapThumb url={item.check_in_snap} label="Check-in photo" /></dd></div><div className="flex justify-between gap-2 min-w-0"><dt className="flex-shrink-0">Check out</dt><dd className="text-right truncate">{item.check_out_time ? displayTime(item.check_out_time) : 'Pending'}</dd></div><div className="flex justify-between gap-2 min-w-0 items-center"><dt className="flex-shrink-0">Check-out photo</dt><dd className="text-right"><SnapThumb url={item.check_out_snap} label="Check-out photo" /></dd></div><div className="flex justify-between gap-2 min-w-0 font-semibold text-slate-800"><dt className="flex-shrink-0">Worked</dt><dd className="text-right truncate">{item.time_worked || '—'}</dd></div><div className="hidden sm:flex justify-between gap-2 min-w-0"><dt className="flex-shrink-0">Last Modified By</dt><dd className="text-right truncate">{displayModifiedBy(item.last_modified_by)}</dd></div>{expanded && <div className="flex flex-col gap-1 min-w-0"><dt className="text-xs sm:text-sm font-medium">Last Modified By</dt><dd className="text-xs sm:text-sm break-words">{displayModifiedBy(item.last_modified_by)}</dd></div>}<div className="sm:hidden mt-2 pt-2 border-t border-slate-100"><button type="button" onClick={() => setExpanded(!expanded)} className="text-xs font-semibold text-[#0F2D52] hover:text-[#173d69] transition-colors">{expanded ? 'View less' : 'View more'}</button></div></dl>{canManage && <div className="-mx-4 sm:-mx-5 -mb-4 sm:-mb-5 mt-4 flex justify-center gap-2 border-t border-slate-100 px-4 sm:px-5 py-3"><Button size="sm" variant="outline" onClick={onEdit} title="Edit"><Pencil className="h-3.5 w-3.5" /></Button><Button size="sm" variant="outline" onClick={onHistory} title="History"><History className="h-3.5 w-3.5" /></Button><Button size="sm" variant="outline" className="border-red-200 text-red-700 hover:bg-red-50 hover:text-red-800" onClick={onDelete} title="Delete"><Trash2 className="h-3.5 w-3.5" /></Button></div>}</div>; }

function TimePicker12h({ value, onChange, disabled, min }: { value: string; onChange: (v: string) => void; disabled?: boolean; min?: string }) {
  return (
    <Input
      type="time"
      value={value}
      ...
      <DialogContent className="sm:max-w-lg">
        ...
        <div className="grid min-w-0 gap-4 py-2">
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <label className="grid gap-1.5 text-xs font-bold uppercase tracking-wider text-slate-500"><span className="flex h-5 items-center">Check-In Time</span><TimePicker12h value={checkInTime} onChange={onCheckIn} disabled={saving} /></label>
            <label className="grid gap-1.5 text-xs font-bold uppercase tracking-wider text-slate-500"><span className="flex h-5 items-center gap-1">Check-Out Time{pending && <span className="text-red-600">*</span>}</span><TimePicker12h value={checkOutTime} onChange={onCheckOut} disabled={saving} /></label>
          </div>
