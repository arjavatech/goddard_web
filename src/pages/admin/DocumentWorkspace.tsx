...
export function DocumentWorkspace({ audience, mode }:Props) {
  ...
  const create = async () => { if (!schoolId || !form.document_name.trim() || (form.target==='selected'&&!selectedIds.length)) return; const created=await createDocumentRequest({school_id:schoolId,audience,document_name:form.document_name,instructions:form.instructions||null,due_date:form.due_date||null,target:form.target, ...(audience==='student'?{child_ids:selectedIds}:{employee_ids:selectedIds})}); await publishDocumentRequest(created.id); setCreateOpen(false); setForm({document_name:'',instructions:'',due_date:'',target:'all'});setSelectedIds([]); await load(); };
  const review = async (next:'approved'|'rejected') => { if (!reviewing || (next==='rejected' && !reason.trim())) return; await reviewDocument(reviewing.id,next,reason||undefined); setReviewing(null); setReason(''); await load(); };
  ...
  return <AdminLayout><main className="container mx-auto px-2 sm:px-4 pb-12 pt-0 sm:pt-12 space-y-6">
    ...
    <Dialog open={createOpen} onOpenChange={(open)=>{setCreateOpen(open);if(open&&schoolId)void fetchDocumentRecipients(schoolId,audience).then(setRecipients)}}><DialogContent><DialogHeader><DialogTitle>Create document request</DialogTitle></DialogHeader><div className="space-y-4"><div><label className="mb-1 block text-sm font-semibold">Document name</label><Input value={form.document_name} onChange={e=>setForm({...form,document_name:e.target.value})} placeholder="Income Certificate"/></div><div><label className="mb-1 block text-sm font-semibold">Instructions</label><textarea value={form.instructions} onChange={e=>setForm({...form,instructions:e.target.value})} className="min-h-24 w-full rounded-xl border border-slate-200 p-3 text-sm" placeholder="Explain what is required..."/></div><div><label className="mb-1 block text-sm font-semibold">Due date (optional)</label><Input type="date" value={form.due_date} onChange={e=>setForm({...form,due_date:e.target.value})}/></div><div><label className="mb-1 block text-sm font-semibold">Recipients</label><select value={form.target} onChange={e=>setForm({...form,target:e.target.value})} className="h-10 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm md:hidden lg:block"><option value="all">All active {audience==='student'?'students':'employees'}</option><option value="selected">Selected {audience==='student'?'students':'employees'}</option></select><div className="hidden md:block lg:hidden"><Select value={form.target} onValueChange={val=>setForm({...form,target:val})}><SelectTrigger className="h-10 w-full rounded-xl border-slate-200 bg-white px-3 text-sm shadow-none focus:ring-0"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="all">All active {audience==='student'?'students':'employees'}</SelectItem><SelectItem value="selected">Selected {audience==='student'?'students':'employees'}</SelectItem></SelectContent></Select></div></div>{form.target==='selected'&&<div className="max-h-40 space-y-1 overflow-y-auto rounded-xl border border-slate-200 p-2">{recipients.map(person=><label key={person.id} className="flex cursor-pointer items-center gap-2 rounded-lg p-2 hover:bg-slate-50"><input type="checkbox" checked={selectedIds.includes(person.id)} onChange={()=>setSelectedIds(ids=>ids.includes(person.id)?ids.filter(id=>id!==person.id):[...ids,person.id])}/><span className="text-sm"><b>{person.name}</b><span className="ml-1 text-slate-500">{person.email}</span></span></label>)}</div>}<div className="flex justify-end gap-2"><Button variant="outline" onClick={()=>setCreateOpen(false)}>Cancel</Button><Button disabled={!form.document_name.trim()||(form.target==='selected'&&!selectedIds.length)} onClick={()=>void create()} className="bg-[#0F2D52] text-white"><Send className="mr-2 h-4 w-4"/>Publish request</Button></div></div></DialogContent></Dialog>
    <Dialog open={Boolean(reviewing)} onOpenChange={open=>!open&&setReviewing(null)}><DialogContent><DialogHeader><DialogTitle>Review document</DialogTitle></DialogHeader>{reviewing&&<div className="space-y-4"><div className="rounded-xl bg-slate-50 p-3"><p className="font-bold">{reviewing.document_name}</p><p className="text-sm text-slate-600">{reviewing.subject_name}</p>{reviewing.latest_file_name&&<Button variant="link" className="mt-2 h-auto p-0" onClick={()=>void openFile(reviewing)}><Eye className="mr-1 h-4 w-4"/>Preview uploaded document</Button>}</div><textarea value={reason} onChange={e=>setReason(e.target.value)} className="min-h-24 w-full rounded-xl border border-slate-200 p-3 text-sm" placeholder="Reason or re-upload instructions (required only when rejecting)"/><div className="flex justify-end gap-2"><Button variant="outline" onClick={()=>void review('rejected')} className="border-rose-200 text-rose-700"><X className="mr-1 h-4 w-4"/>Reject</Button><Button onClick={()=>void review('approved')} className="bg-[#0F2D52] text-white"><Check className="mr-1 h-4 w-4"/>Approve</Button></div></div>}</DialogContent></Dialog>
  </main></AdminLayout>;
}
