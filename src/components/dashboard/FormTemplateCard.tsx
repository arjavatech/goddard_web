  ...
  return <Card className="glass-card h-full">
      <CardHeader className="pb-3">
        <div className="flex items-start justify-between">
          ...
          {template.status && <Badge variant={getStatusVariant(template.status)}>
              {template.status}
            </Badge>}
        </div>
        {template.isRequired && <Badge variant="destructive" className="w-fit">
            Required
          </Badge>}
      </CardHeader>
      ...
      <CardContent className="space-y-4">
        ...
        {template.createdAt && <div className="flex items-center text-xs text-muted-foreground">
