            ...
            return <Card key={item.assignmentId} className="p-5 rounded-2xl border border-slate-100 shadow-xs bg-white flex flex-col justify-between hover:shadow-md transition-all duration-300 hover:-translate-y-1 space-y-4"><CardContent className="p-0">
              <div className="flex items-start gap-2.5 min-w-0 flex-1 pb-3 border-b border-slate-100">
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    <p className="font-bold text-slate-900 text-sm truncate">{item.formName}</p>
                    {item.submissionSource === 'manual_upload' && (
                      <span className="inline-block px-2 py-0.5 text-[10px] font-bold bg-amber-100 text-amber-800 rounded flex-shrink-0">Manual</span>
                    )}
                  </div>
                  <p className="text-[10px] font-extrabold text-slate-400 truncate mt-0.5 uppercase tracking-wider">{subjectName}{kind === 'student' && ` • ${item.classroomName}`}</p>
                </div>
              </div>
