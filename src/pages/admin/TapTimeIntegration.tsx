...
export function TapTimeIntegration() {
  ...
  const [itemsPerPage, setItemsPerPage] = usePageSize('taptime-users', 10);
  const { currentPage, totalPages, paginatedData: paginatedUsers, setCurrentPage } = usePagination({ data: users, itemsPerPage });

  useEffect(() => { setCurrentPage(1); }, [users, setCurrentPage]);
  ...
  return <AdminLayout><main className="min-h-full bg-[#f6f9fd] p-4 sm:p-6 lg:p-8"><div className="mx-auto max-w-7xl space-y-6">
    <header className="flex flex-col sm:flex-row items-start gap-4 sm:gap-3 rounded-2xl border border-slate-100 bg-white p-5 shadow-xs sm:p-6"><div className="rounded-xl bg-[#EFF5FB] p-2.5 text-[#0F2D52]"><Link2 className="h-5 w-5" /></div><div><h1 className="text-xl font-extrabold tracking-tight text-slate-900 sm:text-2xl">TapTime Integration</h1><p className="mt-1 text-xs font-semibold text-slate-400 sm:text-sm">Connect this school once, then manage TapTime from Goddard.</p></div></header>
    <section className="overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-sm">
      {loading ? <Loading size="md" message="Checking TapTime setup…" /> : !schoolId ? <p className="p-5 text-red-700">A school context is required.</p> : <div className="space-y-5">
        <div className="flex flex-col justify-between gap-4 border-b border-slate-100 bg-slate-50/50 p-5 sm:flex-row sm:items-center"><div className="flex items-start sm:items-center gap-3">{status?.configured ? <div className="rounded-xl bg-emerald-50 p-2.5 text-emerald-600"><CheckCircle2 className="h-5 w-5" /></div> : <div className="rounded-xl bg-amber-50 p-2.5 text-amber-600"><CircleAlert className="h-5 w-5" /></div>}<div><p className="text-sm font-bold text-slate-900">{status?.configured ? 'TapTime connected' : 'TapTime setup required'}</p><p className="mt-0.5 text-xs font-semibold text-slate-400">{status?.message}</p></div></div><Button size="sm" variant="outline" className="w-full sm:w-auto" onClick={() => void refresh()} disabled={loading || busy !== null}><RefreshCw className="mr-1.5 h-3.5 w-3.5" />Refresh</Button></div>
        <div className="space-y-5 p-5">
          ...
          {!status?.configured ? <div className="rounded-2xl border border-slate-100 bg-slate-50/60 p-5"><div className="flex flex-col sm:flex-row gap-3 sm:gap-4"><KeyRound className="mt-0.5 h-5 w-5 text-[#0F2D52] hidden sm:block" /><div className="flex-1"><h2 className="flex items-center gap-2 text-sm font-bold text-slate-900"><KeyRound className="h-4 w-4 text-[#0F2D52] sm:hidden" />Connect this school</h2><p className="mt-1 text-sm text-slate-600">Enter the one-time TapTime linking code supplied during onboarding. It is company-scoped, expires automatically, and is never stored in Goddard.</p><div className="mt-4 flex flex-col gap-3 sm:flex-row"><Input value={linkingCode} onChange={e => setLinkingCode(e.target.value)} placeholder="TapTime linking code" className="flex-1" autoComplete="off" /><Button className="w-full sm:w-auto bg-gradient-to-br from-[#0F2D52] to-[#1E4B83] text-white hover:opacity-95 hover:text-white" onClick={() => void redeemCode()} disabled={!linkingCode.trim() || busy !== null}>{busy === 'link' && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}Connect TapTime</Button></div></div></div></div> : <>
            ...
            <div className="space-y-3 rounded-2xl border border-emerald-100 bg-emerald-50/40 p-5"><div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><div><h2 className="text-sm font-bold text-slate-900">TapTime users</h2><p className="mt-1 text-sm text-slate-600">Exact email matches connect automatically. Sync again after correcting an email. Unresolved users are never created automatically.</p></div><Button size="sm" className="w-full sm:w-auto sm:min-w-44 bg-[#0F2D52] text-white shadow-sm hover:bg-[#173d69] hover:text-white" onClick={() => void syncEmailMatches()} disabled={busy !== null}>{busy === 'sync' && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}{busy === 'sync' ? 'Syncing users…' : 'Sync email matches'}</Button></div>{users.length > 0 && <div className="flex justify-end"><PageSizeSelector pageSize={itemsPerPage} onPageSizeChange={handleItemsPerPageChange} /></div>}{paginatedUsers.map(user => <div key={user.user_id} className="flex flex-col gap-3 rounded-xl border border-slate-200 bg-white p-4 sm:flex-row sm:items-center sm:justify-between"><div className="min-w-0 flex-1"><p className="font-semibold text-slate-900">{user.first_name} {user.last_name} <span className="ml-2 text-xs font-medium text-slate-500">{user.role}</span></p><p className="text-sm text-slate-500 break-all">{user.email}</p></div>{user.mapping_status === 'connected' ? <span className="text-sm font-semibold text-emerald-700">Connected</span> : <Button size="sm" className="w-full sm:w-auto flex-none" onClick={() => { setSelectedUser(user); setPin(suggestPin(user.phone)); }} disabled={busy !== null}>Create in TapTime</Button>}</div>)}{users.length > 0 && <div className="overflow-x-auto"><Pagination currentPage={currentPage} totalPages={totalPages} totalItems={users.length} itemsPerPage={itemsPerPage} onPageChange={setCurrentPage} /></div>}</div>
          </>}
    ...
    {selectedUser && <div className="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto bg-slate-950/40 p-4"><div className="w-full max-w-md rounded-2xl bg-white p-5 sm:p-6 shadow-xl my-8"><h2 className="text-lg font-bold text-slate-900">Create TapTime user</h2><p className="mt-1 text-sm text-slate-600">{selectedUser.first_name} {selectedUser.last_name} · {selectedUser.role}</p><label className="mt-5 block text-sm font-semibold text-slate-700">Employee PIN</label><Input className="mt-2" value={pin} onChange={e => setPin(e.target.value.replace(/\D/g, '').slice(0, 10))} placeholder="4-digit PIN" inputMode="numeric" /><p className="mt-2 text-xs text-slate-500">The phone-based value is only a suggestion and can be changed.</p><div className="mt-6 flex flex-col sm:flex-row justify-end gap-3"><Button variant="outline" className="w-full sm:w-auto" onClick={() => setSelectedUser(null)}>Cancel</Button><Button className="w-full sm:w-auto" onClick={() => void createUser()} disabled={!/^\d{4,10}$/.test(pin) || busy !== null}>{busy === 'link' && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}Create user</Button></div></div></div>}
