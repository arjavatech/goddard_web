...
export const createDocumentRequest = (body:unknown) => authedFetch({ method:'POST', url:'/document-requests', data:body }, z.any()) as Promise<DocumentRequest>;
...
export const reviewDocument = (id:string, status:'approved'|'rejected', reason?:string) => authedFetch({ method:'POST', url:`/document-assignments/${id}/review`, data:{status, reason} }, z.any()) as Promise<DocumentAssignment>;
...
export const documentUploadIntent = (id:string, file:File) => authedFetch({ method:'POST', url:`/document-assignments/${id}/upload-intent`, data:{file_name:file.name,content_type:file.type,file_size_bytes:file.size} }, z.any()) as Promise<{storage_key:string;upload_url:string}>;
export const completeDocumentUpload = (id:string, body:unknown) => authedFetch({ method:'POST', url:`/document-assignments/${id}/complete-upload`, data:body }, z.any()) as Promise<DocumentAssignment>;
export const sendDocumentReminders = (schoolId:string, assignmentIds:string[]) => authedFetch({ method:'POST', url:'/document-assignments/reminders', data:{school_id:schoolId,assignment_ids:assignmentIds} }, z.any()) as Promise<DocumentReminderResponse>;
